...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (title) {
            // QUery with index
            boards = await ctx.db.query("boards").withSearchIndex("search_title",
                (q) => q.search("title", title).eq("orgId", args.orgId)).collect()
        } else {
        ...
        const boardsWithFavoriteCollection = boards.map((board) => {
